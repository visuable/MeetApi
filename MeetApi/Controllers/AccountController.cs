...
namespace MeetApi.MeetApi.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login([AllowNull] [FromBody] ViewUser user)
        {
            ...
            if (loginResult) return Ok(model);
        ...
        public async Task<IActionResult> Register([AllowNull] [FromBody] ViewUserRegister user)
        {
            ...
            if (registerStatus) return Ok(model);
        ...
        private async Task<bool> ValidateAndLogin(ViewUser user)
        {
            ...
            if (string.Equals(loginResult, string.Empty)) return false;
            HttpContext.Response.Headers["Authorization"] =
                string.Join(' ', JwtBearerDefaults.AuthenticationScheme, loginResult);
            return true;
        }
    }
}